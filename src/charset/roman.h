#ifndef __NDM_SRC_CHARSET_ROMAN_H__
#define __NDM_SRC_CHARSET_ROMAN_H__

/**
 * Automatically generated by @c convgen
 * from @c ./MAPPINGS/VENDORS/APPLE/ROMAN.TXT
...
static const uint16_t ROMAN_00000020_000000FF_TO_UNI_[] = {
...
static inline long conv_roman_to_uni_(
		...
		uint32_t *cp)
{
	...
	if (0x20 <= b0) {
		const uint16_t code =
			ROMAN_00000020_000000FF_TO_UNI_[b0 - 0x20];
...
static const uint8_t UNI_00000020_000000FF_TO_ROMAN_[] = {
...
static const uint8_t UNI_00000131_00000192_TO_ROMAN_[] = {
...
static const uint8_t UNI_000002C6_000002DD_TO_ROMAN_[] = {
	0xf6, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0xf9, 0xfa, 0xfb, 0xfe, 0xf7, 0xfd
};

static const uint8_t UNI_000003A9_000003C0_TO_ROMAN_[] = {
	0xbd, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xb9
};

static const uint8_t UNI_00002013_000020AC_TO_ROMAN_[] = {
...
static const uint8_t UNI_00002202_00002265_TO_ROMAN_[] = {
...
static const uint8_t UNI_0000FB01_0000FB02_TO_ROMAN_[] = {
	0xde, 0xdf
};

static inline long conv_uni_to_roman_(
		uint32_t cp,
		uint8_t *out,
		const size_t out_bytes)
{
	...
	if (0x20 <= cp && cp <= 0xff) {
		const uint8_t code =
			UNI_00000020_000000FF_TO_ROMAN_[cp - 0x20];
	...
	if (0x0131 <= cp && cp <= 0x0192) {
		const uint8_t code =
			UNI_00000131_00000192_TO_ROMAN_[cp - 0x0131];
	...
	if (0x02c6 <= cp && cp <= 0x02dd) {
		const uint8_t code =
			UNI_000002C6_000002DD_TO_ROMAN_[cp - 0x02c6];
	...
	if (0x03a9 <= cp && cp <= 0x03c0) {
		const uint8_t code =
			UNI_000003A9_000003C0_TO_ROMAN_[cp - 0x03a9];
	...
	if (0x2013 <= cp && cp <= 0x20ac) {
		const uint8_t code =
			UNI_00002013_000020AC_TO_ROMAN_[cp - 0x2013];
	...
	if (0x2202 <= cp && cp <= 0x2265) {
		const uint8_t code =
			UNI_00002202_00002265_TO_ROMAN_[cp - 0x2202];
	...
	if (0xfb01 <= cp && cp <= 0xfb02) {
		*out =
			UNI_0000FB01_0000FB02_TO_ROMAN_[cp - 0xfb01];
...
#endif /* __NDM_SRC_CHARSET_ROMAN_H__ */
