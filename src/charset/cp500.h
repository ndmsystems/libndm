#ifndef __NDM_SRC_CHARSET_CP500_H__
#define __NDM_SRC_CHARSET_CP500_H__

/**
 * Automatically generated by @c convgen
 * from @c ./MAPPINGS/VENDORS/MICSFT/EBCDIC/CP500.TXT
 * Range merge code gap is 128.
 * See http://www.unicode.org/Public/MAPPINGS/ for
 * other charsets.
 **/

#include <assert.h>
#include <stddef.h>
#include <stdint.h>

static const uint8_t CP500_00000000_000000FF_TO_UNI_[] = {
	
};

static inline long conv_cp500_to_uni_(
		const uint8_t *const in,
		const size_t in_bytes,
		uint32_t *cp)
{
	assert (in_bytes > 0);

	*cp =
		CP500_00000000_000000FF_TO_UNI_[*in];

	return 1;
}

static const uint8_t UNI_00000000_000000FF_TO_CP500_[] = {
	
};

static inline long conv_uni_to_cp500_(
		uint32_t cp,
		uint8_t *out,
		const size_t out_bytes)
{
	if (out_bytes == 0) {
		return -1;
	}

	if (cp <= 0xff) {
		*out =
			UNI_00000000_000000FF_TO_CP500_[cp];

		return 1;
	}

	return 0;
}

#endif /* __NDM_SRC_CHARSET_CP500_H__ */
