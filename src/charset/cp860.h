#ifndef __NDM_SRC_CHARSET_CP860_H__
#define __NDM_SRC_CHARSET_CP860_H__

/**
 * Automatically generated by @c convgen
 * from @c ./MAPPINGS/VENDORS/MICSFT/PC/CP860.TXT
...
static const uint16_t CP860_00000080_000000FF_TO_UNI_[] = {
...
static inline long conv_cp860_to_uni_(
		...
		uint32_t *cp)
{
	...
	if (0x80 <= b0) {
		*cp =
			CP860_00000080_000000FF_TO_UNI_[b0 - 0x80];
...
static const uint8_t UNI_000000A0_000000FC_TO_CP860_[] = {
	...
	0x00, 0x97, 0xa3, 0x00, 0x81
};

static const uint8_t UNI_00000393_000003C6_TO_CP860_[] = {
...
static const uint8_t UNI_0000207F_000020A7_TO_CP860_[] = {
...
static const uint8_t UNI_00002219_00002265_TO_CP860_[] = {
...
static const uint8_t UNI_00002320_00002321_TO_CP860_[] = {
	0xf4, 0xf5
};

static const uint8_t UNI_00002500_000025A0_TO_CP860_[] = {
...
static inline long conv_uni_to_cp860_(
		uint32_t cp,
		uint8_t *out,
		const size_t out_bytes)
{
	...
	if (0xa0 <= cp && cp <= 0xfc) {
		const uint8_t code =
			UNI_000000A0_000000FC_TO_CP860_[cp - 0xa0];
	...
	if (0x0393 <= cp && cp <= 0x03c6) {
		const uint8_t code =
			UNI_00000393_000003C6_TO_CP860_[cp - 0x0393];
	...
	if (0x207f <= cp && cp <= 0x20a7) {
		const uint8_t code =
			UNI_0000207F_000020A7_TO_CP860_[cp - 0x207f];
	...
	if (0x2219 <= cp && cp <= 0x2265) {
		const uint8_t code =
			UNI_00002219_00002265_TO_CP860_[cp - 0x2219];
	...
	if (0x2320 <= cp && cp <= 0x2321) {
		*out =
			UNI_00002320_00002321_TO_CP860_[cp - 0x2320];
	...
	if (0x2500 <= cp && cp <= 0x25a0) {
		const uint8_t code =
			UNI_00002500_000025A0_TO_CP860_[cp - 0x2500];
...
#endif /* __NDM_SRC_CHARSET_CP860_H__ */
