#ifndef __NDM_SRC_CHARSET_ICELAND_H__
#define __NDM_SRC_CHARSET_ICELAND_H__

/**
 * Automatically generated by @c convgen
 * from @c ./MAPPINGS/VENDORS/APPLE/ICELAND.TXT
...
static const uint16_t ICELAND_00000020_000000FF_TO_UNI_[] = {
...
static inline long conv_iceland_to_uni_(
		...
		uint32_t *cp)
{
	...
	if (0x20 <= b0) {
		const uint16_t code =
			ICELAND_00000020_000000FF_TO_UNI_[b0 - 0x20];
...
static const uint8_t UNI_00000020_000000FF_TO_ICELAND_[] = {
...
static const uint8_t UNI_00000131_00000192_TO_ICELAND_[] = {
...
static const uint8_t UNI_000002C6_000002DD_TO_ICELAND_[] = {
	0xf6, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0xf9, 0xfa, 0xfb, 0xfe, 0xf7, 0xfd
};

static const uint8_t UNI_000003A9_000003C0_TO_ICELAND_[] = {
	0xbd, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xb9
};

static const uint8_t UNI_00002013_000020AC_TO_ICELAND_[] = {
	0xd0, 0xd1, 0x00, 0x00, 0x00, 0xd4, 0xd5, 0xe2,
	0x00, 0xd2, 0xd3, 0xe3, 0x00, 0x00, 0x00, 0xa5,
...
static const uint8_t UNI_00002202_00002265_TO_ICELAND_[] = {
...
static inline long conv_uni_to_iceland_(
		uint32_t cp,
		uint8_t *out,
		const size_t out_bytes)
{
	...
	if (0x20 <= cp && cp <= 0xff) {
		const uint8_t code =
			UNI_00000020_000000FF_TO_ICELAND_[cp - 0x20];
	...
	if (0x0131 <= cp && cp <= 0x0192) {
		const uint8_t code =
			UNI_00000131_00000192_TO_ICELAND_[cp - 0x0131];
	...
	if (0x02c6 <= cp && cp <= 0x02dd) {
		const uint8_t code =
			UNI_000002C6_000002DD_TO_ICELAND_[cp - 0x02c6];
	...
	if (0x03a9 <= cp && cp <= 0x03c0) {
		const uint8_t code =
			UNI_000003A9_000003C0_TO_ICELAND_[cp - 0x03a9];
	...
	if (0x2013 <= cp && cp <= 0x20ac) {
		const uint8_t code =
			UNI_00002013_000020AC_TO_ICELAND_[cp - 0x2013];
	...
	if (0x2202 <= cp && cp <= 0x2265) {
		const uint8_t code =
			UNI_00002202_00002265_TO_ICELAND_[cp - 0x2202];
...
#endif /* __NDM_SRC_CHARSET_ICELAND_H__ */
